...
async def request_post(req: str = Body()):
    request_eng = consillium_translate(req, src='ru', dest='en')
    request_eng = request_eng
    ...
    #     print(f"Результат:\n")
        ...
        #     print(line)
        # print("\n" + "-" * 50 + "\n")
    result = consillium_translate(request_eng, src='en', dest='ru')
    return result

async def consillium_translate(text, src, dest):
    translator = Translator()
    try:
        translated = await translator.translate(text, src, dest)
