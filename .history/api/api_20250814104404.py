...
async def request_post(req: str = Body()):
    request_eng = consillium_translate(req, src='ru', dest='en')  # без await
    with MedicalTextGenerator(biobert_tokenizer, biobert_model, llm) as generator:
        ...
        print("Результат:\n")
    result_ru = consillium_translate(result, src='en', dest='ru')  # без await
    return result_ru

def consillium_translate(text: str, src: str, dest: str) -> str:
    translator = Translator()
    try:
        # Используем именованные параметры и без await
